(function () {
    ...
    function MarketingInboxCtrl($rootScope, $scope, $uibModal, request, langs, $location, logger) {
        ...
        $scope.partList = 1;
        $scope.dialogsContainer = document.getElementById('dialogs-container')
        ...
        $scope.get = function($offset) {
            request.send('/dialogs/' + $offset, {}, function (data) {
                $scope.dialogs = [...$scope.dialogs, ...data];
        ...
        /* $scope.getVonage = function(offset) {
            request.send('/dialogs?offset='+offset, {}, function (data) {
                $scope.dialogs = [...$scope.dialogs, ...data];
                if ($scope.dialogs.length) {
                    var temp = $location.path().split('/');
                    if (temp[3]) {
                    ...
                    $scope.setVonageClient($scope.activeClient);
                }
            }, 'get');
        }; */

        $scope.initScroll = function() {
            var el = document.getElementById('dialogs-container');
            el.addEventListener('scroll', function (e) {
                if (e.target.scrollTop.toFixed(0) * 1 + e.target.clientHeight >= e.target.scrollHeight) {
                    $scope.get(50 * $scope.partList)
                    $scope.partList += 1
                }
            })
        };
        ...
        $scope.setVonageClient = function(client) {
            $location.path('/vonage/list/' + client.id, false);
            $scope.activeClient.id = client.id;
            $scope.activeClient.firstname = client.firstname;
            $scope.activeClient.lastname = client.lastname;
            $scope.showPhonesBox = ! $scope.showPhonesBox;
        ...
        $scope.maxOneText = function() {
            return 160 - ' Txt STOP to OptOut'.length - ($scope.user.company_name.length + 2);
        };
        ...
        $scope.send = function() {
            ...
            var date = new Date();
            var post_mas = {
                'text': $scope.messages_text.replace('\n', ' '),
                'time': {
                    'hour': date.getHours()
                }
            };
            ...
            $scope.sent = true;
            request.send('/dialogs/send/' + $scope.activeClient.id, post_mas, function (data) {
                if (data) {
